...
# 从环境变量获取账号密码和 TG 配置
EMAIL_B        = os.environ.get("KATABUMP_EMAIL_B") or ""    # 登录邮箱
PASSWORD_B     = os.environ.get("KATABUMP_PASSWORD_B") or "" # 账号密码
...
def _solve_altcha(sb) -> bool:
    ...
    # 先检查是否已自动通过
    if sb.execute_script(_BLTCHA_SOLVED_JS):
        print("✅ ALTCHA 已自动通过")
        return True
    ...
    coords = None
    try:
        coords = sb.execute_script(_BLTCHA_EXPAND_JS)
    except Exception:
        pass
    ...
    for attempt in range(3):
        ...
        try:
            new_coords = sb.execute_script(_BLTCHA_EXPAND_JS)
            if new_coords:
                coords = new_coords
        except Exception:
            pass
